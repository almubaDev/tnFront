...
export default function PocionesDineroScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      if (navigationHistory.peek() !== 'PocionesDinero') {
        console.log('Current navigation history in PocionesDinero:', navigationHistory.history);
      }
    ...
    try {
      ...
      if (perfilResponse.ok) {
        ...
        if (perfilData.tiene_suscripcion) {
          // Cargar las pociones
          const response = await fetchWithAuth('/pociones/?categoria=dinero');
          if (response.ok) {
          ...
          const comprasResponse = await fetchWithAuth('/mis-pociones/');
  ...
  const comprarPocion = async (pocion) => {
      ...
      showAlert(
        ...
        [
          ...
          { text: "Ver planes", onPress: () => {
            ...
            navigationHistory.push('PocionesDinero');
    ...
    try {
      ...
      const response = await fetchWithAuth('/comprar-pocion/', {
        method: 'POST',
